...
# convery the warped image to grayscale, then threshold it
# to give it that 'block to white' paper effect
warped = cv2.cvtColor(warped, cv2.COLOR_BGR2GRAY)
T = cv2.threshold_local(warped, 11, offset = 10, method = "gaussian")
warped = ( warped > T).astype("uint8") * 255

# show the original and scanned images
print("STEP 3: Apply perspctive transform")
cv2.imshow("Original", imutils.resize(orgin, height = 650 ))
